...
title: Bigish data
reading: ""
notes: "<a href='hw/hw5.html'>HW 5</a> - due <strike>Thurs 4/14</strike> Sun 4/17 by 11 pm"
output: 
...
```{r, eval=FALSE}
system.time(read.csv("/data/Sta323/nyc_parking/NYParkingViolations.csv"))
...
```{r, eval=FALSE}
library(data.table)
system.time({nyc = fread("/data/Sta323/nyc_parking/NYParkingViolations.csv")})

## Read 9100278 rows and 43 (of 43) columns from 1.673 GB file in 00:00:35
##    user  system elapsed 
##  34.422   0.709  35.155 
...
```{r, eval=FALSE}
library(readr)
system.time({nyc = read_csv("/data/Sta323/nyc_parking/NYParkingViolations.csv")})
...
## Classes ‘tbl_df’, ‘tbl’ and 'data.frame':    654437 obs. of  4 variables:
##  $ row     : int  2647 3792 4001 4002 4003 4004 4005 4006 4019 4059 ...
##  $ col     : chr  "Violation Legal Code" "Violation Legal Code" "Violation Legal Code" "Violation Legal Code" ...
...
## 1      1361929741  FCJ5493                 NY        PAS 1970-12-18             20
## 2      1366962000  63540MC                 NY        COM 1971-02-02             46
## 3      1356906515  GFM1421                 NY        PAS 1971-09-18             40
## 4      1342296217  FYM5117                 NY        SRF 1971-09-18             21
## 5      1342296199  95V6675                 TX        PAS 1971-09-18             21
## 6      1342296187  GCY4187                 NY        SRF 1971-09-18             21
## 7      1337077380  18972BB                 NY        999 1971-10-10             14
## 8      1364523796  WNJ4730                 VA        PAS 1973-04-05             14
## 9      1359914924  68091JZ                 NY        COM 1973-07-22             46
## 10     1355498326  EWV4127                 NY        PAS 1973-08-12             21
## ..            ...      ...                ...        ...        ...            ...
## Variables not shown: Vehicle.Body.Type (chr), Vehicle.Make (chr), Issuing.Agency (chr),
##   Street.Code1 (int), Street.Code2 (int), Street.Code3 (int), Vehicle.Expiration.Date
##   (int), Violation.Location (chr), Violation.Precinct (int), Issuer.Precinct (int),
...
The parking data we've looked at does not contain the amount of the fine issued, but we do know the Violation Code. A second data set is available (`/data/Sta323/nyc_parking/fine_definition.csv`) that lists each violation code along with a description and the fine amount (in and outside of Manhattan).
